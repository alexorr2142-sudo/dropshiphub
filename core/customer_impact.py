...
def _draft_customer_message(row: dict, impact_category: str) -> str:
    ...
    if impact_category == "No tracking":
        return (
            ...
            "We’re still waiting on tracking details from our shipping partner. "
            "As soon as tracking is available, we’ll send it right away. "
    ...
    if impact_category == "Carrier exception":
        return (
            ...
            "We’re actively investigating and will follow up with the next update ASAP. "
            "If you’d like, we can also offer alternative resolution options."
        )
    ...
    if impact_category == "Partial shipment":
        return (
            ...
            "We’ll send tracking for each package as it becomes available. "
            "Thanks for your patience!"
        )
    ...
    if impact_category == "Late risk":
        ...
        return (
            f"Hi there — quick update on {order_ref}{sku_part}. "
            "Your shipment is taking a bit longer than expected, and we’re working to confirm the latest ship/tracking update. "
            f"{supplier_bit}{extra} "
            "We’ll follow up with the next update as soon as we have it. "
            "Thanks for your patience!"
        )
    ...
    return (
        f"Hi there — quick update on {order_ref}{sku_part}. "
        "We’re reviewing your shipment status and will share the next update as soon as possible. "
        "Thanks for your patience!"
    )
...
def build_customer_impact_view(exceptions: pd.DataFrame, max_items: int = 50) -> pd.DataFrame:
    ...
    Requires exceptions with at least: order_id, issue_type, explanation, next_action, Urgency (optional)
    """
    ...
    # Only keep rows likely to matter to customers
    blob = (
    ...
    # Sort: urgency first, then category
    if "Urgency" in out.columns:
        # if Urgency is categorical it sorts well; else we map
        order = {"Critical": 0, "High": 1, "Medium": 2, "Low": 3}
        out["_u"] = out["Urgency"].map(order).fillna(9)
        out = out.sort_values(["_u", "impact_category", "order_id"], ascending=True).drop(columns=["_u"], errors="ignore")
    else:
        out = out.sort_values(["impact_category", "order_id"], ascending=True)
