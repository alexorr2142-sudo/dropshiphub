...
# ✅ Issue Tracker persistence + maintenance + load-run open filtering
...
def _import_or_stop():
    ...
    # ✅ SLA Escalations + Issue Tracker (your new feature)
...
- Audit trail (coming soon)
    ...
    # ============================================================
    # Issue Tracker Maintenance (Prune old resolved + Clear resolved)
    # ============================================================
    st.divider()
...
# Key rule to prevent "lost email preview":
#   ✅ ALWAYS build followups_open by filtering followups_full (preserves subject/body/email cols)
# ============================================================
followups_full = followups  # full snapshot (includes resolved/notes columns, etc.)
followups_open = followups  # open snapshot (unresolved only)
escalations_df = pd.DataFrame()

if MOD["render_sla_escalations"] is not None:
    try:
        # Your UI renders the editor and returns (escalations_df, followups_full, followups_open)
        escalations_df, followups_full_from_ui, _ = MOD["render_sla_escalations"](
            line_status_df=line_status_df,
            followups=followups,
            promised_ship_days=int(default_promised_ship_days),
        )
        # Use whatever the UI returned as "full" if present
        followups_full = followups_full_from_ui if isinstance(followups_full_from_ui, pd.DataFrame) else followups_full
    except TypeError:
        # Backward compatibility if your render_sla_escalations signature differs
    ...
    # ✅ Build open by filtering full using the issue tracker store
    store = IssueTrackerStore()
    issue_map = store.load()
    ...
    # ✅ Downstream ops should use open only
...
if MOD["workspace_root"] is not None:
    ...
    with st.sidebar:
        ...
        if st.button("💾 Save this run", key="btn_save_run"):
            run_dir = MOD["save_run"](
                ...
                # ✅ Save FULL followups so you keep history/notes
                followups=followups_full if isinstance(followups_full, pd.DataFrame) else (followups if isinstance(followups, pd.DataFrame) else pd.DataFrame()),
    ...
    if st.session_state.get("loaded_run"):
        ...
        # ✅ Load FULL followups snapshot (history)
        ...
        # ✅ Rebuild OPEN followups from FULL using issue tracker store (preserves email cols)
        store = IssueTrackerStore()
        issue_map = store.load()
...
# ============================================================
# Supplier Follow-ups (Copy/Paste Ready)  ✅ (kept + protected)
# ============================================================
st.divider()
st.subheader("Supplier Follow-ups (Copy/Paste Ready)")

followups_for_ops = followups_open if isinstance(followups_open, pd.DataFrame) else (followups if isinstance(followups, pd.DataFrame) else pd.DataFrame())
...
if followups_for_ops is None or followups_for_ops.empty:
    ...
    with cdl2:
        if isinstance(followups_full, pd.DataFrame) and not followups_full.empty:
            st.download_button(
    ...
    # ✅ Email preview stays available as long as subject/body exist
    if (
        MOD["copy_button"] is not None
        and "supplier_name" in followups_for_ops.columns
        and "body" in followups_for_ops.columns
        and len(followups_for_ops) > 0
    ):
        st.divider()
        ...
        # Helpful hint if email fields are missing
        missing = []
        for col in ["supplier_name", "body"]:
            if col not in followups_for_ops.columns:
                missing.append(col)
        if missing:
            st.caption(f"Email preview hidden: missing column(s) {', '.join(missing)}")
