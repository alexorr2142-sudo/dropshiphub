...
with tab1:
    ...
    if followups_for_ops is None or followups_for_ops.empty:
        ...
        # ✅ FIX #2: Supplier accountability builder kwargs mismatch
        # Preferred signature: build_supplier_accountability_view(scorecard, top_n=10)
...
with tab2:
    ...
    if customer_impact is None or customer_impact.empty:
        ...
        if render_customer_comms_ui is not None:
            try:
                # ✅ FIX (compat): call only accepted kwargs (some versions need ws_root, etc.)
                ...
                # last resort: try minimal positional
                try:
